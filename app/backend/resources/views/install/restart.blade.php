...
<div class="bg-slate-50 rounded-lg p-4 mb-2">
    <p class="text-sm font-medium text-navy mb-3 flex items-center gap-2">
        <iconify-icon icon="heroicons:server-stack" style="font-size: 16px" class="text-teal"></iconify-icon>
        Status
    </p>
    <div class="space-y-2">
        <div class="flex items-center gap-2 text-sm">
            ...
            @if ($mounted)
                <span class="badge badge-success gap-1 text-xs">
                    <iconify-icon icon="heroicons:check" style="font-size: 14px"></iconify-icon>
                    Mounted
                </span>
            @else
...
@if ($postInstall)
    ...
    <form action="{{ route('dashboard') }}" method="GET" class="mt-6">
        <button type="submit" class="btn btn-primary w-full">
            <iconify-icon icon="heroicons:arrow-path" style="font-size: 16px"></iconify-icon>
            Reload page
        </button>
    </form>
@else
